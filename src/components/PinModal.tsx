...
export const PinDisplay = ({ pin, peerName, onClose }: PinDisplayProps) => (
  <motion.div
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
    exit={{ opacity: 0 }}
  ...
  >
    ...
    >
      ...
      <p className="text-center text-white/50 text-xs mb-3">
        Enter this PIN on <span className="text-white">{peerName}</span>
      </p>
      ...
      <div className="flex justify-center gap-2 mb-5">
        {pin.split('').map((digit, i) => (
          <motion.div
            ...
            className="w-9 h-11 rounded-xl bg-white/[0.07] border border-white/10
              flex items-center justify-center text-xl font-bold text-white font-mono"
          >
            {digit}
          </motion.div>
        ))}
      ...
      <button
        onClick={onClose}
        className="w-full py-2.5 rounded-xl text-sm text-white/50 hover:text-white
